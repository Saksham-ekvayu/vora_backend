...
const frameworkUploadValidation = [
...
const getFrameworkByIdSchema = Joi.object({
  id: frameworkIdValidator(),
});

const deleteFrameworkSchema = Joi.object({
  id: frameworkIdValidator(),
});

const getFrameworksQuerySchema = Joi.object({
  page: pageValidator(),
...
const getFrameworkByIdValidation = (req, res, next) => {
  const { error } = getFrameworkByIdSchema.validate({ id: req.params.id });
  if (error) {
    return res.status(400).json({
      success: false,
      message: error.details[0].message,
...
const deleteFrameworkValidation = (req, res, next) => {
  const { error } = deleteFrameworkSchema.validate({ id: req.params.id });
  if (error) {
    return res.status(400).json({
      success: false,
      message: error.details[0].message,
...
const getFrameworksQueryValidation = (req, res, next) => {
  const { error } = getFrameworksQuerySchema.validate(req.query);
  if (error) {
    return res.status(400).json({
      success: false,
      message: error.details[0].message,

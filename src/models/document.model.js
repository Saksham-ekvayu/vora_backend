...
documentSchema.statics.getByType = function(documentType) {
  return this.find({ documentType, isActive: true }).populate('uploadedBy', 'name email role');
};

// Static method to get user documents
documentSchema.statics.getUserDocuments = function(userId) {
  return this.find({ uploadedBy: userId, isActive: true }).populate('uploadedBy', 'name email role');
};

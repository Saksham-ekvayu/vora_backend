...
/**
 * User AI Service - AI operations for user frameworks
 */
...
class UserAIService {
  ...
  /**
   * Upload user framework to AI service
  ...
  async uploadFramework(filePath) {
    try {
      ...
      const response = await addRequest("/user/framework/upload", {
        method: "POST",
        data: formData,
        headers: {
          ...formData.getHeaders(),
  ...
  startBackgroundMonitoring(uuid, frameworkId, onMessage) {
    try {
      const aiWsUrl = `${AI_BASE_URL.replace(
        "http",
        "ws"
      )}/user/framework/extract-controls/${uuid}`;
  ...
  async checkProcessingStatus(uuid) {
    try {
      ...
      const response = await addRequest(`/user/framework/status/${uuid}`, {
        method: "GET",
      });
...
const userAIService = new UserAIService();
